...
sealed interface DonaResult<out T> {
    data class Success<T>(val data: T) : DonaResult<T>
    data class Error(val failure: DonaFailure) : DonaResult<Nothing>
}
...
sealed interface DonaFailure {
    ...
    data class Unreachable(override val message: String?, override val cause: Throwable? = null) : DonaFailure

    /** The DPU responded but rejected the credentials. */
    data class InvalidCredentials(override val message: String? = null, override val cause: Throwable? = null) :
        DonaFailure

    /** The DPU responded with an unexpected payload/status that we could not parse. */
    data class UnexpectedResponse(override val message: String?, override val cause: Throwable? = null) : DonaFailure

    /** No house/session configured yet. */
    data class NotAuthenticated(override val message: String? = "Not logged in") : DonaFailure {
        override val cause: Throwable? = null
    }

    data class Unknown(override val message: String?, override val cause: Throwable? = null) : DonaFailure
}

inline fun <T, R> DonaResult<T>.map(transform: (T) -> R): DonaResult<R> = when (this) {
