...
class AuthRepositoryImplTest {
    ...
    @Test
    fun `successful login on the first attempt returns a session and does not try the local IP`() = runTest {
    ...
    @Test
    fun `falls back to the local IP when the DNS address is unreachable`() = runTest {
        coEvery {
    ...
    @Test
    fun `unknown username fails without creating a session`() = runTest {
